...
def prune_resnet18_conv_layer(model, layer_index, filter_index):
	...
	new_down_conv = None
	if layer_index == 0:
		conv = model.features._modules.get(str(layer_index))
		next_conv =  model.features._modules.get('4')[0].conv1
	...
	if layer_index > 0 and layer_index < 5:
		...
		pt=layer_index%2
		if pt==1:
			conv = model.features._modules.get(str(3+tt))[kt].conv1
			next_conv =  model.features._modules.get(str(3+tt))[kt].conv2
		else:
			if kt==0:
				conv = model.features._modules.get(str(3+tt))[kt].conv2
				next_conv =  model.features._modules.get(str(3+tt))[kt+1].conv1
			else:
				conv = model.features._modules.get(str(3+tt))[kt].conv2
				next_conv =  model.features._modules.get(str(3+tt+1))[0].conv1
				downin_conv =  model.features._modules.get(str(3+tt+1))[0].downsample[0]
	...
	if layer_index > 4 and layer_index < 9:
		...
		if pt==1:
			...
			if kt==0:
				conv = model.features._modules.get(str(3+tt))[kt].conv2
				next_conv =  model.features._modules.get(str(3+tt))[kt+1].conv1
				downout_conv =  model.features._modules.get(str(3+tt))[kt].downsample[0]
			else:
				conv = model.features._modules.get(str(3+tt))[kt].conv2
				next_conv =  model.features._modules.get(str(3+tt+1))[0].conv1
				downin_conv =  model.features._modules.get(str(3+tt+1))[0].downsample[0]
	...
	if not next_conv is None:
		if layer_index ==0:
			...
			model.features._modules.get(str(4))[0].conv1 = next_new_conv
		else:
			if pt==1:
				model.features._modules.get(str(3+tt))[kt].conv1 = new_conv
				model.features._modules.get(str(3+tt))[kt].conv2 = next_new_conv
				#if tt > 1:
				...
				if kt==0:                                
					model.features._modules.get(str(3+tt))[kt].conv2 = new_conv
					model.features._modules.get(str(3+tt))[kt+1].conv1 = next_new_conv
					if tt > 1:
						ds = torch.nn.Sequential(
							*(replace_layers(model.features._modules.get(str(3+tt))[kt].downsample, i, [0], \
								[new_down_conv]) for i, _ in enumerate(model.features._modules.get(str(3+tt))[kt].downsample)))
						model.features._modules.get(str(3+tt))[kt].downsample = ds
				else:                                
					model.features._modules.get(str(3+tt))[kt].conv2 = new_conv
					model.features._modules.get(str(3+tt+1))[0].conv1 = next_new_conv
					#if tt == 1:
					ds = torch.nn.Sequential(
						*(replace_layers(model.features._modules.get(str(3+tt+1))[0].downsample, i, [0], \
							[next_downin_conv]) for i, _ in enumerate(model.features._modules.get(str(3+tt+1))[0].downsample)))
					model.features._modules.get(str(3+tt+1))[0].downsample = ds
